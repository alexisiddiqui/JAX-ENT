#!/usr/bin/env python3
"""Selected-model analysis plots for 4_aSyn conditions."""
...
def main() -> None:
    p = argparse.ArgumentParser(description="Plot selected-model summaries for 4_aSyn")
    ...
    p.add_argument("--cluster-populations-csv", default=None)
    a = p.parse_args()
    ...
    summary_raw = pd.read_csv(a.before_csv)
    pop_cols = [
        c
        for c in summary_raw.columns
        if c.endswith("_mean")
        and not any(x in c for x in ["_rank_", "_transformed_", "_percentile_"])
        and c.replace("_mean", "_std") in summary_raw.columns
        and c.startswith("cluster_")
    ]
    if pop_cols:
        keep = [c for c in ["ensemble", "split_type", "loss_function", "score_metric"] if c in summary_raw.columns]
        pop_summary = summary_raw[keep + pop_cols + [c.replace("_mean", "_std") for c in pop_cols]].copy()
        plot_cluster_populations(
            pop_summary,
            a.output_dir,
