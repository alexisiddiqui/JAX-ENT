...
BV_REG_VALUES_STR="0.0,0.25,0.5,0.75,1.0"

DIR_NAME="_optimise_quick_test_FIGURE_SIGMA_100_FUNCTIONAL"
N_STEPS=100
INITIAL_STEPS=0
INITIAL_LR=1.0
LEARNING_RATE=1.0
...
FUNCTIONAL_MAXENT_LOSSES_STR="ElasticNet,ElasticMAE,KLD_PF,Work_Fitting,Work_Magnitude"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --functional-maxent-losses)
      FUNCTIONAL_MAXENT_LOSSES_STR="$2"; shift 2;;
    --functional-maxent-losses=*)
      FUNCTIONAL_MAXENT_LOSSES_STR="${1#*=}"; shift;;
    --dir-name)
      DIR_NAME="$2"; shift 2;;
    --dir-name=*)
      DIR_NAME="${1#*=}"; shift;;
    ...
    -h|--help)
      echo "Usage: $0 [--ensembles a,b] [--losses x,y] [--split-types s,t] [--maxent-values a,b,c] [--bvreg-values a,b] [--functional-maxent-losses ElasticNet,KLD_PF] [--dir-name name] [--n-steps N] [--initial-steps M] [--initial-learning-rate X] [--learning-rate Y] [--ema-alpha Z] [--forward-model-scaling S] [-j|--jobs N]"
      exit 0;;
    *)
      break;;
  esac
...
echo "BV reg values (raw): $BV_REG_VALUES_STR"
echo "Functional MaxEnt losses (raw): $FUNCTIONAL_MAXENT_LOSSES_STR"
...
IFS=',' read -r -a FUNCTIONAL_MAXENT_LOSSES <<< "$FUNCTIONAL_MAXENT_LOSSES_STR"
    ...
    for SPLIT in "${SPLIT_TYPES[@]}"; do
      ...
      for MAXENT in "${MAXENT_VALUES[@]}"; do
        for BV_REG in "${BV_REG_VALUES[@]}"; do
          for FUNC_LOSS in "${FUNCTIONAL_MAXENT_LOSSES[@]}"; do
            echo "    Maxent: $MAXENT, BV reg: $BV_REG ($FUNC_LOSS)"
            # --- Changed: ensure no more than PARALLEL_JOBS are running concurrently ---
            wait_for_slot
            python optimise_ISO_TRI_BI_splits_Functional_maxENT \
              --ensemble "$ENSEMBLE" \
              --loss-function "$LOSS" \
              --maxent-range "$MAXENT,$MAXENT" \
              --bvreg-range "$BV_REG,$BV_REG" \
              --functional-maxent-loss "$FUNC_LOSS" \
              --split-types "$SPLIT" \
              --n-steps "$N_STEPS" \
              ...
              > "${OPT_OUTPUT_DIR}/logs/${ENSEMBLE}_${LOSS}_maxent${MAXENT}_bvreg${BV_REG}_${FUNC_LOSS}_split${SPLIT}.log" 2>&1 &
          done
        done
      done
...
python "${ANA_DIR}/weights_validation_ISO_TRI_BI_2D_BV.py" \
  --results-dir "$OPT_OUTPUT_DIR" \
