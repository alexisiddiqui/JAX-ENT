"""
[Script Name] optimise_ISO_TRI_BI_splits_maxENT.py

[Brief Description of Functionality]
Performs optimization sweeps for the MoPrP ensembles (AF2_MSAss, AF2_filtered) using Maximum Entropy (MaxEnt)
regularization. It fits the model parameters (including frame weights) to match the experimental HDX data
across multiple data splits and MaxEnt scaling values.
The fitting runs in serial for each combination of ensemble, loss function, split type, and MaxEnt value.

Requirements:
    - Featurised data in `_featurise/` (`features_AF2_MSAss.npz`, etc.)
    - Data splits in `_datasplits/`
    - Covariance matrix in `../../data/_MoPrP_covariance_matrices/Sigma.npz`
...
Usage:
    ...
    python optimise_ISO_TRI_BI_splits_maxENT.py --ensemble AF2_filtered --loss-function mcMSE --split-types spatial

    # Run with specific MaxEnt range:
    python optimise_ISO_TRI_BI_splits_maxENT.py --maxent-range 1,5

Output:
    - Optimization history files (`.hdf5`) saved in `_optimise_quick_test_{timestamp}/` (or specified --output-dir).
      Files named: `{ensemble}_{loss}_{split_type}_split{idx}_maxent{val}_results.hdf5`
"""
    ...
    for split_type in split_types:
        ...
        for split_idx, (train_data, val_data) in enumerate(splits):
            ...
            for maxent_value in maxent_values:
                ...
                try:
                    loss_config = LossConfig(primary_loss=LOSS_NAME_MAP[loss_name])
                    run_optimization(
