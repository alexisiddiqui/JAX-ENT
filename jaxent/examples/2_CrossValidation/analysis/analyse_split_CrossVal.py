...
def plot_uptake_heatmap(
    peptides: list[pt.Partial_Topology],
    dfracs: np.ndarray,
    title: str,
    filename: str,
    ...
    # Reshape dfracs if it's 3D with the last dimension being 1
    if dfracs.ndim == 3 and dfracs.shape[-1] == 1:
        dfracs = dfracs.squeeze(axis=-1)
    sns.heatmap(
        dfracs,
        xticklabels=False,
...
def plot_combined_uptake_comparison(
    train_peptides: list[pt.Partial_Topology],
    train_dfracs: np.ndarray,
    val_peptides: list[pt.Partial_Topology],
    ...
    # Reshape train_dfracs if it's 3D with the last dimension being 1
    if train_dfracs.ndim == 3 and train_dfracs.shape[-1] == 1:
        train_dfracs = train_dfracs.squeeze(axis=-1)
    sns.heatmap(
        train_dfracs,
        xticklabels=False,
    ...
    # Reshape val_dfracs if it's 3D with the last dimension being 1
    if val_dfracs.ndim == 3 and val_dfracs.shape[-1] == 1:
        val_dfracs = val_dfracs.squeeze(axis=-1)
    sns.heatmap(
        val_dfracs,
        xticklabels=False,
...
def main():
    ...
    output_base_dir = os.path.join(os.path.dirname(__file__), "_analysis_split_CrossVal")
    os.makedirs(output_base_dir, exist_ok=True)

    if not os.path.exists(base_dir):
        print(f"Data directory not found: {base_dir}")
        print("Please run the `splitdata_CrossVal.py` script first.")
        return
