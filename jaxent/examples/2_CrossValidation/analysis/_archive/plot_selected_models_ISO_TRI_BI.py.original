...
def run_fixed_effects_for_score(df, value_col, score_name, output_dir, transform_func=None, has_loss=True):
    ...
    try:
        ...
        return None, None # Return None for both results and local_df
...
def run_fixed_effects_analysis(df_before, df_diff, df_minimax, output_dir):
    ...
    if res_summary is not None: results_dict['Mean_Recovery'] = (res_summary, res_full)
    ...
    if res_summary is not None: results_dict['CV'] = (res_summary, res_full)
    ...
    if res_summary is not None: results_dict['Regret'] = (res_summary, res_full)
    ...
    if res_summary is not None: results_dict['Minimax'] = (res_summary, res_full)
