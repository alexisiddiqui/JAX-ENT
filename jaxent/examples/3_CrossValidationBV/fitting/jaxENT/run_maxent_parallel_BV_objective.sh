...
DIR_NAME="_optimise_quick_FIGURE_SIGMA_5000_lr1.0_BV_objectve_scale1.0"
N_STEPS=5000
LEARNING_RATE=1.0
LR_ADJUSTMENT=on
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      echo "Usage: $0 [--ensembles a,b] [--losses x,y] [--split-types s,t] [--maxent-values a,b,c] [--bvreg-values a,b] [--bv-reg-losses L1,L2] [--dir-name name] [--n-steps N] [--learning-rate Y] [--lr-adjustment on|off] [--frame-average-impl tensordot|legacy_sum] [--step-chunk-size N] [-j|--jobs N]"
      exit 0;;
    *)
      break;;
  esac
...
echo "BV reg losses (raw): $BV_REG_LOSSES_STR"
...
python -m jaxent.examples.common.manifest --output-dir "$OPT_OUTPUT_DIR" --example 3 --ensembles "$ENSEMBLES_STR" --losses "$LOSSES_STR" --split-types "$SPLIT_TYPES_STR" --maxent-values "$MAXENT_VALUES_STR" --bv-values "$BV_REG_VALUES_STR" --bv-reg-functions "$BV_REG_LOSSES_STR" --learning-rate "$LEARNING_RATE" --lr-adjustment "$LR_ADJUSTMENT" --frame-average-impl "$FRAME_AVERAGE_IMPL" --step-chunk-size "$STEP_CHUNK_SIZE" --n-steps "$N_STEPS" --jobs "$PARALLEL_JOBS" --timepoints-file "${DIR_WD}/../../../2_CrossValidation/data/_MoPrP/moprp.times"
