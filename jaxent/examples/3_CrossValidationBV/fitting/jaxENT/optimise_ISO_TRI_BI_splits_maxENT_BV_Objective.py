...
from optimise_fn import run_optimise_ISO_TRI_BI_maxENT_BV_objective
...
def get_bv_reg_function(reg_name: str):
    """Get BV regularization function by name."""
    from jaxent.src.opt.losses import model_params_L2_loss, model_params_L1_loss    

    regs = {
        "L1": model_params_L1_loss,
        "L2": model_params_L2_loss,
    }
    if reg_name not in regs:
        raise ValueError(f"Unknown BV regularization function: {reg_name}. Available: {list(regs.keys())}")
...
def run_maxent_sweep(
    ...
    bv_reg_function = get_bv_reg_function(bv_reg_name)
        ...
        for split_idx, (train_data, val_data) in enumerate(splits):
            ...
            for maxent_value in maxent_values:
                for bv_reg_value in bv_reg_values:
                    ...
                    try:
                        run_optimise_ISO_TRI_BI_maxENT_BV_objective(
