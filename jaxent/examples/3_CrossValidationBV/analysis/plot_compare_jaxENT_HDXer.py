...
def _annotate_bar_heights(ax, fmt="{:.1f}", offset=1.5):
    """
    Annotate bar heights on the plot.

    Args:
        ax: The axis object to annotate
        fmt: Format string for the annotation text
        offset: Vertical offset for the annotation text
    """
    for patch in ax.patches:
...
def main():
    ...
    """
    # Define paths
    base_dir = "/home/alexi/Documents/JAX-ENT/jaxent/examples/3_CrossValidationBV"
    hdxer_path = os.path.join(base_dir, "data/_HDXer/selected_metrics_all.csv")
    jaxent_path = os.path.join(base_dir, "analysis/_analysis__optimise_quick_test_FIGURE_SIGMA_5000_lr1.0_BV_objectve_scale1.0__20251118_154208/selected_metrics/selected_metrics_all.csv")
