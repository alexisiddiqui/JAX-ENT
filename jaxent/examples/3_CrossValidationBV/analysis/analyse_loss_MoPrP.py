...
# Ensemble colors
ENSEMBLE_COLORS = {"AF2_MSAss": "#1f77b4", "AF2_filtered": "#ff7f0e"}
...
def plot_best_model_comparisons(df: pd.DataFrame, output_dir: str):
    ...
    # Create separate figures for losses vs other metrics
    for metric, label in metrics:
        ...
        for idx, loss_func in enumerate(loss_functions):
            ...
            if not loss_df.empty:
                ...
                # Use log scale for losses and KL/JS divergence
                if "loss" in metric or "divergence" in metric.lower():
                    ax.set_yscale("log")
            else:
        ...
        plt.suptitle(f"Best Model Comparison: {label}", fontsize=16, fontweight="bold")
        plt.tight_layout()
        ...
        for loss_func in loss_functions:
            ...
            if "maxent_value" in df.columns:
                ...
                if len(subset) > 0:
                    ...
                    # Plot training loss
                    color = ensemble_colors.get(ensemble, default_color)
        ...
        for loss_func in loss_functions:
            if "maxent_value" in df.columns:
                ...
                if len(subset) > 0:
                    ...
                    std_stats = stats.dropna(subset=["convergence_rate"])
