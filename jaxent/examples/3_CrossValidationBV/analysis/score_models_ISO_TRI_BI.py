"""
score_models_ISO_TRI_BI.py  (Exp3 — CrossValidationBV)

Computes scores (MSE, dMSE, work metrics, recovery %) for optimized models
from the 2D BV regularization sweep.
...
Requirements:
    ...
    - Features (_featurise/)
    - Clustering results
    - Target state ratios JSON (state_ratios.json)

Usage:
    python jaxent/examples/3_CrossValidationBV/analysis/score_models_ISO_TRI_BI.py \
        --processed-data-dir "$PROCESSED_DIR" \
        --state-ratios-json "${DIR_WD}/../../2_CrossValidation/analysis/state_ratios.json"
"""
...
def _load_target_ratios(state_ratios_path: str) -> dict | None:
    """Load target state ratios from a state_ratios.json file."""
    if not os.path.exists(state_ratios_path):
        print(f"State ratios file not found: {state_ratios_path}")
        return None
    try:
        with open(state_ratios_path) as f:
            ratios_data = json.load(f)
        pops = ratios_data["fractional_populations"]
        target_ratios = {
...
def main():
    parser = argparse.ArgumentParser(
        description="Calculate scores and metrics from processed Exp3 BV-sweep optimization results."
    )
    parser.add_argument(
        "--processed-data-dir",
    ...
    parser.add_argument(
        "--clustering-dir",
        default="../2_CrossValidation/analysis/_MoPrP_analysis_clusters_feature_spec_AF2_test/clusters",
        help="Directory containing cluster assignment subdirectories.",
    )
    parser.add_argument(
        "--state-ratios-json",
        default="../2_CrossValidation/analysis/state_ratios.json",
    ...
    if args.absolute_paths:
        ...
        state_ratios_path = os.path.abspath(os.path.join(script_dir, args.state_ratios_json))
    ...
    ensemble_clustering_map = config.scoring.ensemble_clustering_map

    # Load target ratios from JSON
    ...
    # First pass: collect run metadata (with extra BV reg groups)
    ...
    # Convert bv_reg_str to numeric and rename bv_reg_fn for clarity
