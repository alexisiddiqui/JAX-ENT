...
def get_loss_function(loss_name: str):
    ...
    losses = {"mcMSE": hdx_uptake_mean_centred_MSE_loss, "MSE": hdx_uptake_MSE_loss, "Sigma_MSE": hdx_uptake_sigma_MSE_loss}
...
def save_timing_to_csv(results: dict, output_dir: str, ensemble: str, loss_name: str, slice_size: int):
    """
    Save timing data for a maxent sweep to a CSV file.
    
    Args:
        results: Results dictionary from run_maxent_sweep
        output_dir: Directory to save the CSV
        ensemble: Ensemble name
        loss_name: Loss function name
        slice_size: Slice size used
    """
    csv_path = os.path.join(output_dir, f"timing_{ensemble}_{loss_name}_slice{slice_size}.csv")
    
    os.makedirs(output_dir, exist_ok=True)
    
    with open(csv_path, 'w', newline='') as csvfile:
        fieldnames = ['ensemble', 'loss_name', 'slice_size', 'split_type', 'split_idx', 'maxent_value', 
                      'status', 'elapsed_time_s', 'run_name']
        ...
        for run_detail in results['run_details']:
            writer.writerow({
                ...
                'split_type': run_detail['split_type'],
                'split_idx': run_detail['split_idx'],
                'maxent_value': run_detail['maxent_value'],
                'status': run_detail['status'],
                'elapsed_time_s': run_detail['elapsed_time'],
                'run_name': run_detail['run_name'],
            })
...
def run_maxent_sweep(
    ...
    slice_size: int = 1,  # NEW: add slice_size parameter
) -> dict:
    """
    ...
    Args:
        ...
        slice_size: Slice size to use for features (default: 1)
    Returns:
    ...
    feature_path = os.path.join(features_dir, f"sliced_{slice_size}", f"features_{ensemble.lower()}.npz")
    ...
    cov_matrix_path = "_covariance_matrices_sigma/ISO_BI_Sigma_weighted.npz"

    cov_matrix_data = jnp.load(os.path.join(os.path.dirname(__file__), cov_matrix_path))["Sigma_inv"]  # Scale down for numerical stability
    ...
    # NEW: Save timing data to CSV
    timing_output_dir = os.path.join(output_base_dir, "timing_reports")
    save_timing_to_csv(results, timing_output_dir, ensemble, loss_name, slice_size)
...
def run_all_combinations(
    ...
    for i, (ensemble, loss_name) in enumerate(combinations, 1):
        ...
        try:
            result = run_maxent_sweep(
                ...
                slice_size=slice_size,  # NEW: pass through
            )
    ...
    # NEW: Aggregate all timing data into a single summary CSV
    if output_base_dir:
        summary_csv_path = os.path.join(output_base_dir, "timing_summary_all.csv")
        with open(summary_csv_path, 'w', newline='') as csvfile:
            ...
            for result in all_results:
                for run_detail in result['run_details']:
                    writer.writerow({
        ...
        print(f"\nSaved aggregate timing summary to: {summary_csv_path}")
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Slice size for feature selection (default: 1). Use sliced_X subdirectory in _featurise.",
    )
    ...
    if args.ensemble is not None and args.loss_function is not None:
        ...
        results = run_maxent_sweep(
            ...
            slice_size=args.slice_size,  # NEW: pass through
        )
