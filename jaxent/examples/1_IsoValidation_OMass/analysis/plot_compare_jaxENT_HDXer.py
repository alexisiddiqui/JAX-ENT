...
def load_and_harmonize_data(hdxer_path, jaxent_path):
    ...
    print(f"  Split types in combined data: {sorted(combined_df['split_type'].unique())}")
    print(f"  Split names in combined data: {sorted(combined_df['split_name'].unique())}")
...
def _annotate_bar_heights(ax, decimal_places=1):
    """
    Annotate bar heights on the plot for recovery percentages.
    
    Args:
        ax: The axis object to annotate
        decimal_places: Number of decimal places for the annotation text
    """
    # Iterate over bars in the barplot
    for bar in ax.patches:
        # Get the height of the bar
        height = bar.get_height()
        
        # Annotate with the recovery percentage
        ax.annotate(
...
def plot_comparison_by_experiment(data, output_dir, analysis_split_types=None):
    ...
    for col_idx, experiment in enumerate(available_experiments):
        ...
        if not exp_data.empty:
            ...
            if ax.get_legend() is not None:
                ax.get_legend().remove()
            # NOTE: No text annotations for KL-divergence bars — annotations are only applied to recovery (%) plots.
        else:
            ax.set_visible(False)
...
def plot_comparison_by_ensemble(data, output_dir, analysis_split_types=None):
    ...
    for col_idx, ensemble in enumerate(available_ensembles):
        ...
        if not ens_data.empty:
            ...
            # Styling
            display_name = "ISO-BiModal" if ensemble == "ISO_BI" else "ISO-TriModal"
            ax.set_title(f"TeaA | {display_name}", fontsize=22, fontweight="bold",
...
def main():
    ...
    """
    # Define paths
    base_dir = "/home/alexi/Documents/JAX-ENT/jaxent/examples/1_IsoValidation_OMass"
    hdxer_path = os.path.join(base_dir, "data/_HDXer/selected_metrics_all.csv")
    jaxent_path = os.path.join(base_dir, "analysis/_analysis_optimise_FIGURE_SIGMA_5000__20251103_145956/selected_metrics/selected_metrics_all.csv")
