...
def run_analysis_on_subset(
    ...
    # Select predictors
    exclude_cols = {target_metric, "convergence_value", "maxent_value", "split_idx"}
...
def main():
    parser = argparse.ArgumentParser(
        description="Perform comprehensive linear effects modelling on JAX-ENT optimization scores."
    )
    parser.add_argument(
        "--scores-csv-path",
    ...
    for filter_best_convergence in modes:
        ...
        if args.analyze_subsets and 'loss_function' in df_clean_base.columns:
            unique_losses = df_clean_base['loss_function'].unique()
            print(f"Found loss functions: {unique_losses}")
            for loss_func in unique_losses:
                datasets_to_analyze[loss_func] = df_clean_base[df_clean_base['loss_function'] == loss_func]
...
def plot_scatter_and_distributions(
    ...
    for metric in predictor_cols:
        ...
        fig, axes = plt.subplots(
            len(ensembles), 5, figsize=(32, 5 * len(ensembles))
        )
        if len(ensembles) == 1:
...
def plot_metric_distributions(
    ...
    """Create distribution plots for metrics by ensemble and split type."""
    ensembles = sorted(data["ensemble"].unique())

    for metric in data.columns:
        if metric not in output_dir:
            continue

        fig, axes = plt.subplots(
            1, len(ensembles), figsize=(7 * len(ensembles), 5), sharey=False
        )
        if len(ensembles) == 1:
            axes = [axes]
        ...
        for idx, ensemble in enumerate(ensembles):
            ...
            if "split_type" in df_ens.columns:
                ...
                ax.hist(df_ens[metric].dropna(), bins=20, alpha=0.7, color="steelblue")

            ax.set_xlabel(metric, fontsize=12)
            ax.set_ylabel("Frequency", fontsize=12)
            ax.set_title(f"{ensemble}", fontsize=13, fontweight="bold")
            ax.legend(fontsize=10)
            ax.grid(alpha=0.3)

        plt.tight_layout()
        output_path = os.path.join(output_dir, f"05_distribution_{metric}.png")
        fig.savefig(output_path, dpi=300, bbox_inches="tight")
        print(f"Saved: {output_path}")
        plt.close()
...
def plot_model_selection_performance(
    ...
    # Ensure necessary columns exist for grouping
    if 'loss_function' in df.columns:
        group_cols = ['ensemble', 'loss_function', 'split_type', 'split_idx']
        x_axis = 'loss_function'
    else:
        # If loss_function is missing, create a dummy one
        df_plot = df.copy()
        df_plot['loss_function'] = 'All'
        group_cols = ['ensemble', 'loss_function', 'split_type', 'split_idx']
        x_axis = 'loss_function'
        df = df_plot
    
    # Filter out groups that don't have the target metric
    df_clean = df.dropna(subset=[target_metric]).copy()
    ...
    for metric in predictor_cols:
        ...
        try:
            ...
            g.set_axis_labels("Loss Function", target_metric)
...
def calculate_and_save_correlations(
    ...
    """
    Calculate Pearson correlations between target metric and predictor metrics.
    ...
    for metric in predictor_cols:
        ...
        valid = df[[metric, target_metric]].dropna()
        if len(valid) < 2:
            continue
        corr, p_value = stats.pearsonr(valid[metric], valid[target_metric])
