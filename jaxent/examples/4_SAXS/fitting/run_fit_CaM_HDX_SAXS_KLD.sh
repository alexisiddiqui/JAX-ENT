...
#   --hdx-split-indices 0,1 Comma-separated indices   (default: 0,1,2)
#   --saxs-split-indices 0,1 Comma-separated indices  (default: 0,1,2)
...
HDX_SPLIT_INDICES="0,1,2"
SAXS_SPLIT_INDICES="0,1,2"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --hdx-split-indices)  HDX_SPLIT_INDICES="$2"; shift 2 ;;
        --saxs-split-indices) SAXS_SPLIT_INDICES="$2"; shift 2 ;;
...
IFS=',' read -ra HDX_SPLIT_IDX_ARRAY  <<< "$HDX_SPLIT_INDICES"
IFS=',' read -ra SAXS_SPLIT_IDX_ARRAY <<< "$SAXS_SPLIT_INDICES"
...
for _ in "${TARGET_ARRAY[@]}"; do
    for _ in "${HDX_SPLIT_IDX_ARRAY[@]}"; do
        for _ in "${SAXS_SPLIT_IDX_ARRAY[@]}"; do
...
echo "  HDX indices:  ${HDX_SPLIT_INDICES}"
echo "  SAXS indices: ${SAXS_SPLIT_INDICES}"
...
for TARGET in "${TARGET_ARRAY[@]}"; do
    for HDX_IDX in "${HDX_SPLIT_IDX_ARRAY[@]}"; do
        for SAXS_IDX in "${SAXS_SPLIT_IDX_ARRAY[@]}"; do
            for SAXS_W in "${SAXS_W_ARRAY[@]}"; do
                for HDX_W in "${HDX_W_ARRAY[@]}"; do
                    for MAXENT in "${MAXENT_ARRAY[@]}"; do
                        ...
                        LOG_NAME="HDX_SAXS_${TARGET}_h${HDX_IDX}_s${SAXS_IDX}_maxent${MAXENT}_w${SAXS_W}_v${HDX_W}"
                        ...
                        python "${SCRIPT_DIR}/fit_CaM_HDX_SAXS_KLD.py" \
                            --target         "$TARGET" \
                            --hdx-split-index "$HDX_IDX" \
                            --saxs-split-index "$SAXS_IDX" \
                            --saxs-weight    "$SAXS_W" \
                            --hdx-weight     "$HDX_W" \
