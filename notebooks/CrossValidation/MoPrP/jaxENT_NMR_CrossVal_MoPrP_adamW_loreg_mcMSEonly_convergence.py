...
def setup_simulation(
    ...
    opt_settings = OptimiserSettings(
        name="test", n_steps=5000, learning_rate=1e-3, convergence=1e-8
    )
    ...
    resnames = [str(res.resname) for res in test_universe.residues]
    print(resnames)
    # extract single letter code

    for idx, seg in enumerate(segs):
        seg_top = Partial_Topology(
            fragment_index=idx,
            chain=test_universe.residues[int(seg[0])].segid,
            residue_start=int(seg[0]),
            residue_end=int(seg[1]),
            fragment_sequence=str(idx),
        )

        exp_top_segs.append(seg_top)
    # print(exp_top_segs)
    ...
    # Create features
    exp_data = [HDX_peptide(dfrac=_dfrac, top=top) for _dfrac, top in zip(dfracs, exp_top_segs)]
...
def run_MAE_max_ent_optimization_replicates(
    ...
    convergence_rates = [1e-6, 5e-6, 1e-7, 5e-7, 1e-8, 1e-9, 1e-10, 1e-15]
        ...
        for j in range(n_replicates):
            try:
                ...
                new_params = Simulation_Parameters(
                    ...
                    forward_model_weights=jnp.array([1, 0, 0], dtype=jnp.float32),
                    forward_model_scaling=jnp.array([1.0, 1.0, 1.0], dtype=jnp.float32),
                    normalise_loss_functions=jnp.zeros(3, dtype=jnp.float32),
                )
                ...
                opt_settings.convergence = convergence_rates[j]
...
def main():
    ...
    n_replicates = len(convergence_rates)
    ...
    base_output_dir = os.path.join(base_output_dir, f"{protein}/jaxENT/mCMSEonly_highconvergence")
