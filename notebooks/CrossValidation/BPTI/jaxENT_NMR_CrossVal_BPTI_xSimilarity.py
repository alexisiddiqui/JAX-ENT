...
from jaxent.src.models.HDX.BV.forwardmodel import BV_input_features, BV_model, BV_model_Config
...
def run_MAE_max_ent_optimization_replicates(
    ...
    pairwise_Pf_cosine_similarity: Array,
        ...
        for j in range(n_replicates):
            try:
                ...
                new_params = Simulation_Parameters(
                    ...
                    forward_model_weights=jnp.array([0.5, 10 / (j + 1), 1.0], dtype=jnp.float32),
                    forward_model_scaling=jnp.array([1.0, 0.1, 1.0], dtype=jnp.float32),
                    normalise_loss_functions=jnp.array([0.0, 0.0, 0.0], dtype=jnp.float32),
                )
                ...
                _, opt_result = run_optimise(
                    ...
                    initialise=True,
                    optimizer=optimiser,
                    data_to_fit=(dataset, pairwise_Pf_cosine_similarity, pf_prior_data),
                    config=opt_settings,
                    forward_models=models,
                    indexes=[0, 0, 0],
                    loss_functions=[
...
def main():
    ...
    # pick script dir
    base_output_dir = os.path.dirname(os.path.realpath(__file__))

    base_output_dir = os.path.join(base_output_dir, f"{protein}/jaxENT/xSimilarity")
    ...
    for regularization_name, regularization_fn in regularization.items():
        ...
        history_dict = run_MAE_max_ent_optimization_replicates(
            ...
            datasets=datasets,
            models=models,
            pairwise_Pf_cosine_similarity=pairwise_Pf_cosine_similarity,
