...
# Function to compute RMSD between each frame in a trajectory and reference structures
def compute_rmsd_to_references(trajectory_path, topology_path, reference_paths):
    """
    Compute RMSD between each frame in a trajectory and reference structures.
    ...
    reference_paths : list
        Paths to reference structure files
    ...
    """
    # Load trajectory
    traj = Universe(topology_path, trajectory_path)
    ...
    for j, ref_path in enumerate(reference_paths):
        ...
        mobile = Universe(topology_path, trajectory_path)
        reference = Universe(ref_path)
...
def create_synthetic_data(
    ...
    # Frame weights computation (always needs to be done)
    ...
    # Create frame weights based on state ratios
    ...
    # Create synthetic data using the frame weights
    models = [BV_model(bv_config)]
...
def run_quick_auto_validation(
    ...
    new_params = Simulation_Parameters(
        ...
        model_parameters=[bv_config.forward_parameters],
        forward_model_weights=jnp.array([0.16, 250.0, 0.1], dtype=jnp.float32),
        forward_model_scaling=jnp.ones(3, dtype=jnp.float32),
...
if __name__ == "__main__":
    ...
    base_output_dir = os.path.join(os.path.dirname(__file__), "TeaA")
