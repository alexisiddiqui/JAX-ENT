...
def run_L2_optimization(
    ...
    for i, dataset in enumerate(datasets):
        ...
        try:
            ...
            output = new_simulation.outputs[0]
            prior_pfs = output.uptake.T

            prior_data = [
                HDX_peptide(dfrac=_prior_df, top=top)
                for _prior_df, top in zip(prior_pfs, dataset.top)
            ]
            pf_prior_data = ExpD_Dataloader(data=prior_data)
            prior_test_sparse_map = create_sparse_map(features[0], dataset.top, pf_prior_data.data)
            ...
            _, opt_result = run_optimise(
                ...
                initialise=False,
                optimizer=optimiser,
                data_to_fit=(dataset, params, pf_prior_data),
                config=opt_settings,
                forward_models=models,
                indexes=[0, 0, 0],
                loss_functions=[
...
def run_quick_auto_validation(
    ...
    new_params = Simulation_Parameters(
        ...
        forward_model_weights=jnp.array([10.0, regularisation_scale, 0.1], dtype=jnp.float32),
...
if __name__ == "__main__":
    ...
    regularisation_scales = [0, 0.01, 0.1, 1.0, 10.0, 100.0]
    regularisation_scales = [1.0, 2.0, 5.0, 10.0, 20.0, 100.0]
    ...
    for regularisation_scale in regularisation_scales:
        ...
        base_output_dir = os.path.join(
            ...
            "normalisation_MaxENT_MAE",
            f"TeaA_simple_{regularisation_scale}_adamW",
        )
