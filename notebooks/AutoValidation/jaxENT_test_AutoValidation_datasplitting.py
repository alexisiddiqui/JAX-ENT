...
from jaxent.src.opt.losses import exp_frame_weight_consistency_loss, hdx_uptake_l2_loss
...
def create_synthetic_data(
    ...
    synthetic_data = [
        HDX_peptide(dfrac=np.asarray(_dfrac[i]), top=dfrac_topology[i])
    ...
    for i, dataset in enumerate(datasets):
        ...
        try:
            ...
            _, opt_result = run_optimise(
                ...
                forward_models=models,
                indexes=[0, 0],
                loss_functions=[hdx_uptake_l2_loss, exp_frame_weight_consistency_loss],
            )
...
def setup_simulation(
    ...
    opt_settings = OptimiserSettings(name="test", n_steps=500, convergence=1e-5)
...
if __name__ == "__main__":
    ...
    output_dir = os.path.join(base_output_dir, "datset_splitting_test")
    os.makedirs(output_dir, exist_ok=True)
    ...
    for i in range(3):
        train_data = datasets[i].train.data
        val_data = datasets[i].val.data

        split_fig = plot_split_visualization(train_data, val_data, synthetic_data)
